...
# (4 entrance directions x 2 layout variants = 8 designs per style)
# ---------------------------------------------------------------------------
DIRECTIONS = ["North", "South", "East", "West"]
LAYOUT_VARIANTS = ["Balanced Center Setup", "Cosy Corner Concept"]

# Rough rotation offsets (radians) applied to base furniture layouts so that
# duplicated objects aren't literally identical across every direction.
# This is a placeholder heuristic — for true per-layout furniture arrangement,
# ask Gemini for distinct object sets per layout variant (see notes below).
DIRECTION_ROTATION_OFFSET = {
    "North": 0.0,
    "East": -1.57,
    "South": 3.14,
    "West": 1.57
...
def build_direction_layout_prompt(style: str, room_type: str, direction: str, layout: str, description: str) -> str:
    """
    Builds a prompt for ONE specific direction + layout combination.
...
- High-end architectural visualization, realistic shadows, no people, clean composition, no text or watermarks."""
    ...
    async def generate_style_variants(
        ...
        """
        Generates 8 images for ONE style: 4 entrance directions x 2 layout variants.
        Runs the Pollinations calls concurrently (bounded by a semaphore) so the whole
        batch doesn't take 8x as long as a single request.
        """
        import httpx

        combos = [(direction, layout) for direction in DIRECTIONS for layout in LAYOUT_VARIANTS]

        # Create all 8 Design rows up front so we have IDs to use as filenames
        designs = []
        for direction, layout in combos:
            design = DesignModel(
                project_id=project_id,
                style=style,
                direction=direction,
        ...
        async def fetch_one(design, direction, layout):
            async with semaphore:
                ...
                try:
                    ...
                    print(f"Error generating {style}/{direction}/{layout} for design {design.id}: {e}")
                    design.image_url = pollinations_url

        await asyncio.gather(*[
            fetch_one(design, direction, layout)
        ...
        # Populate 3D objects for each design. If base_objects came from Gemini,
        # reuse them per combo with a per-direction rotation offset so scenes
        # aren't perfectly identical. For genuinely distinct furniture arrangement
        # per layout variant, extend the Gemini schema to return separate object
        # sets per layout (see notes in chat) and swap them in here instead.
        if not base_objects:
            base_objects = [
                {"object_type": "floor", "position_x": 0.0, "position_y": 0.0, "position_z": 0.0, "rotation": 0.0, "scale": 1.0, "material": "wood_light"},
                {"object_type": "wall", "position_x": 0.0, "position_y": 1.5, "position_z": -4.0, "rotation": 0.0, "scale": 1.0, "material": "#f0ede6"},
                {"object_type": "sofa", "position_x": 0.0, "position_y": 0.5, "position_z": -2.0, "rotation": 0.0, "scale": 1.0, "material": "#d1c7bd"}
            ]
    ...
    async def analyze_and_generate_styles(self, project_id: uuid.UUID, file: UploadFile, db: Session):
        """
        Multimodal AI generation service.
        Uploads the file (image/video) to Gemini API and uses Gemini to segment
        and recommend style variations + 3D object positions.

        For each of the 5 styles, generates 8 images (4 entrance directions x 2 layout
        variants) via generate_style_variants(), so the total output is
        5 styles x 8 images = 40 designs per project.
        """
        project = db.query(ProjectModel).filter(ProjectModel.id == project_id).first()
        room_type_hint = project.room_type if project and project.room_type else "Living Room"
        ...
        # -------------------------------------------------------------
        # For each style, generate 8 images: 4 directions x 2 layouts,
        # instead of a single image per style.
        # -------------------------------------------------------------
        for style in styles:
            style_data = result.get("styles", {}).get(style, {})
            gemini_desc = style_data.get("description", "")
            base_objects = style_data.get("objects", [])

            variant_designs = await self.generate_style_variants(
                project_id=project_id,
                style=style,
            ...
            generated_designs.extend(variant_designs)
